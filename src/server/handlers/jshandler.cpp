...
PWS::JSHandler::JSHandler(Poco::URI uri, Poco::Logger::Ptr console_logger) : uri(uri), console_logger(console_logger) {
	//
}

void PWS::JSHandler::handleRequest(Poco::Net::HTTPServerRequest& request, Poco::Net::HTTPServerResponse& response) {
	std::stringstream ss; std::fstream file;

	Poco::Path path{ uri.getPath() };

	ss << config_app_folder << "/public" << this->uri.getPath(); // this->uri.getPath() вернёт /js/abob.js
